...
[CreateAssetMenu(fileName = "Skills", menuName = "Skills/Alvi/AlviSkillOne")]
public class AlviSkillOne : SkillData
{
    ...
    {
        float temp = skillParent.GetAttack() * 1.2f;
        ...
        for (int i = 0; i < controlList.Count; i++)
        {
            if (controlList[i].GetState() != characterStates.DEAD)
            {
                ...
                if(rand <= 15)
                {
                    Poison poison = controlList[i].AddComponent<Poison>();
                    controlList[i].AddEffect(poison, 1);
                }
            }
        }
    }
